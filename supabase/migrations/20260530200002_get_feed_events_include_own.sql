-- Fix: include viewer's own events so notification deep-links to own posts resolve.
-- Previously filtered to friends-only; now includes actor_user_id = viewer.
...
BEGIN
  ...
  WHERE fe.event_type = 'set_started'
    AND (
      fe.actor_user_id = viewer
      OR fe.actor_user_id IN (
        SELECT CASE
