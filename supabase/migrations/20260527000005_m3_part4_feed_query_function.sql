-- M3 PART 4: Feed events query function.
-- Returns enriched set_started events for the viewer's friends.
-- Follows the get_inbox_threads / get_friend_favourites SECURITY DEFINER pattern.
-- friendships uses user_a/user_b (confirmed via schema audit, not requester_id/addressee_id).
...
BEGIN
  ...
  SELECT
    ...
    NULL::text AS actor_avatar_url,  -- profiles.avatar_url not yet in schema (HANDOVER §17)
    s.name AS set_name,
