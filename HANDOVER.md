...
- **Marketplace PART 4 (on-demand "find on eBay" from set/card pages, real eBay logo SVG, possibly a settings-page region picker for profiles.country)** — next eBay piece. Watch for: if users complain "I keep seeing junk cards at the top", the answer is probably value-based shuffle weighting (high-value items earlier), not a full filter UI re-add.
