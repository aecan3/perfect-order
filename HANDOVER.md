...
- **Phase 4 Door B — anonymous browse + collection migration (commits `51c3716`, `d43aaf5`, `c1951a7`, `8f1a018`, `17fcb50`, `437f8e1`, `4f47932`):** Full anonymous acquisition funnel, end-to-end. Public catalog (`/sets`, `/set/[setId]`, `/friend/[handle]`) open to logged-out users; 2-item anonymous tab bar (Sets, Sign Up Free). Anonymous ticks saved to `ms_anon_entries` localStorage with per-entry `setId`. CTA count + value updates live via `ms-anon-entries-changed` custom event. Threshold modals (`card_threshold`) at 5/15/30/50 cards. On signup: `verifyOtp` → `/auth/confirm` catch-all migration block. On signin: catch-all migration block regardless of intent. Both paths call `POST /api/anonymous-migration`, which batch-upserts `collection_entries` AND `user_sets` (both are required — see §17 gotcha). localStorage cleared on full match; `ms_show_restore_toast` sessionStorage triggers a 5s lime bar on the landing set page. Service worker bumped v22→v23 with `/auth/*` and `/api/*` bypass rules. **Verified end-to-end on production: test7 account, 15 cards migrated to me4, MY SETS displays me4 correctly.**
  ...
  **Cross-context confirm behaviour (confirmed 4 Jun 2026, test10+test11):** ~~If the user ticks cards in one browser context (e.g. incognito tab, iOS PWA) and then opens the email confirmation link in a *different* context (mail app → default browser), `localStorage` is empty at confirm time and the migration POST is silently skipped — resulting in an empty-looking account.~~ This is not a code bug; it is a browser storage partition. ~~**Recovery:** the `/login` catch-all runs on the user's next sign-in in the original context and migrates whatever is in `localStorage` at that point (proven working).~~ ~~**Future UX:** add a contextual hint on an empty MY SETS page ("Still missing your cards? Open the link in this browser, or sign in here to recover them") to reduce confusion.~~ — **SUPERSEDED 13 Jun 2026:** cross-device confirm now recovers the collection from the server-side stash; see the cross-device onboarding block in §18. The "empty-looking account / recovered on next sign-in" limitation no longer applies.

- **Phase 5 — Edition mode dropdown (4 Jun 2026, commits `61b4525`, `4a7df0a`, `72c0eda`, `2fc9aea`):** `edition_mode` column on `user_sets` (CHECK: any/all/first_edition/unlimited/shadowless, DEFAULT 'any', migration `20260604020000`). Owner set page: edition dropdown visible when set has ≥2 edition prefixes; persisted to `user_sets.edition_mode` for authed users, `ms_anon_entries.setModes[setId]` for anon. Friend page: ephemeral `useState("any")`, resets per visit, no persistence. Mode-aware counts (headline, section headers, partial badge, missing filter), value calculations ('any' = cheapest-per-slot all-in; 'all' = sum all printings; single-edition = that edition only; owned value always = actual checked printing prices). `master_completions` gate is mode-agnostic (always slot-level). GM section unaffected. Both migration callers (`/login`, `/auth/confirm`) POST `setModes: parsed.setModes || {}` so anon edition preference survives signup. **`useCollectionState` merge fix (`2fc9aea`)**: `saveToStorage` now reads existing blob and merges before writing — preserves `setModes` (written by `setAnonEditionMode` outside the hook's awareness). Without this, any tick after a mode switch would overwrite `setModes`. **Verified on device (4 Jun 2026):** modern set = no dropdown; WOTC set = dropdown; mode switching changes counts/values; master_completion mode-agnostic; friend page ephemeral.
  **Anon edition-mode migration — VERIFIED IN PRODUCTION (test11, 5 Jun 2026, Unlimited mode):** Switched to Unlimited mode first → ticked 5 Fossil cards → signed up → confirmed same browser → DB shows `user_sets.edition_mode = 'unlimited'` and 5 `collection_entries` for the correct `unlimited_holofoil` printings. The `useCollectionState` merge fix (`2fc9aea`) is the critical enabler: without it, any tick after a mode switch would overwrite `setModes`, and the migration caller would POST `setModes: {}`, landing `edition_mode = 'any'`. Non-default value (unlimited) is the strongest meaningful verification.
  Key files: `lib/edition-utils.js` (matchesEdition, getAnonEditionMode, setAnonEditionMode), `lib/hooks/useCollectionState.js` (saveToStorage merge), `app/set/[setId]/page.js`, `app/friend/[handle]/[setId]/page.js`, `app/api/anonymous-migration/route.js`.

- **Base Set edition + shadowless seeding + pricing (4 Jun 2026, migration `20260604030000`):** 204 new printings for base1 — first_edition(87) + first_edition_holofoil(15) + shadowless(87) + shadowless_holofoil(15). INSERT…SELECT from existing unlimited/unlimited_holofoil rows (ON CONFLICT DO NOTHING). display_order: 1st Ed.=3, 1st Ed. Holo=4, Shadowless=5, Shadowless Holo=6 (v1 tradeoff: Shadowless Holo trails Unlimited Holo at display_order=1; fix by renumbering unlimited_holofoil→7 in a future migration). All assertions passed: 87/15/87/15 per type, orphan join=0, slot count=102, alex base1=102/102 unchanged.
  Pricing: single PPT fetch of setId=1663 (the "Shadowless" TCGPlayer set, which carries both 1st Ed and Unlimited/Shadowless variant families). first_edition/first_edition_holofoil priced from variants["1st Edition"]/["1st Edition Holofoil"]; shadowless/shadowless_holofoil from variants["Unlimited"]/["Unlimited Holofoil"]. Hit rates: first_edition 86/87, first_edition_holofoil 15/15, shadowless 86/87, shadowless_holofoil 15/15. The 1 miss per non-holo type is base1-8 (no PPT variant data; existing unlimited row is also null). Charizard #4 spot-check: fe_holo=$10,000 ✓, shadowless_holo=$2,146 ✓, unlimited_holo unchanged ✓.
  `pptVariantPrice` in `app/api/refresh-prices/route.js` patched to handle first_edition → "1st Edition", first_edition_holofoil → "1st Edition Holofoil", shadowless → "Unlimited", shadowless_holofoil → "Unlimited Holofoil" via `firstConditionPrice` (first condition entry's `.price`). Scripts: `scripts/price-base1-editions.mjs`, `scripts/price-wotc-fe-gaps.mjs`.

- **Fossil + Neo gap-holo pricing (4 Jun 2026):** 29 unpriced first_edition/first_edition_holofoil rows patched via `scripts/price-wotc-fe-gaps.mjs` (ptcgio → PPT resolution). base3=20/20, gym2=1/1, neo2=2/2, neo4=5/5, neo3=1/1. All 29 priced. Notable prices: Umbreon (neo2-13 fe_holo)=$1,500; Shining Magikarp (neo3-66 fe_holo)=$549; Shining Kabutops (neo4-108 fe_holo)=$1,000. Zero remaining unpriced first_edition/first_edition_holofoil rows except base1-8 (known PPT gap).
...
- **First-party analytics (founder dashboard groundwork):** Custom event pipeline distinct from Vercel Analytics/Sentry and from the Feed feed_events system. `analytics_events` table + `lib/track.js` (sendBeacon-first, anon_id + session_id + cookie-derived user_id) + allowlist in `lib/analytics-events.js`. `/api/track` ingest (public, service-role), `/api/analytics/identify` for anon→user stitch, `analytics_identity` table. Hooks live: page_view, set_opened, card_added, threshold_shown, auth_required_shown (favourites-only — camera surface deliberately silent), nav_away_warned, signup_started, signup_completed (confirm-only). REMAINING: loop-entry hooks (want_list_viewed, friend_link_viewed, trade_invite_viewed, referral_landing) and the dashboard query layer + UI. Note: set_opened double-fires under dev React strict mode only — not a prod issue.
- **Referral links (publish these in social bios / video descriptions):** `/tt` → TikTok (`source: tiktok`), `/ig` → Instagram (`source: instagram`), `/yt` → YouTube (`source: youtube`). Each renders null, fires `referral_landing { source }`, then redirects to `/sets`. `/start` (tiktok) and `/start2` (instagram) are kept as **legacy aliases** (still attribute correctly — do not delete; cached links exist). All five are exact-match `PUBLIC_PATHS` entries in `proxy.js`. `dashboard_acquisition` groups completed signups by these `source` values, so a new channel appears automatically once events arrive.
...
- ~~**Wire Sentry error capture — DONE 5 Jun 2026.**~~ Errors-only (traces and replay off). Instrumented: anonymous migration HTTP errors + count mismatches (login + confirm), user_sets upsert failure, localStorage save errors (QuotaExceededError carved out), all five refresh-prices source catches. Server capture verified live 5 Jun 2026; client tunnel verified by analysis (SW GET-only guard + /monitoring bypass), first production client event pending.
...
- ~~**Trade-invite signup gap (analytics 2b-followup):** the propose_trade sub-branch in `/auth/confirm` does no migration and fires no signup_completed — a trade-invite signup confirming via that branch gets neither cards nor the event. Trade-matching is the core differentiator, so highest-value analytics gap to close. NOT done.~~ **DONE (13 Jun 2026, commit `b1c818d`)** — extracted the catch-all's migration + identity stitch + signup_completed into a shared `runPostConfirmMigrationAndAnalytics()` helper; the propose_trade branch now awaits it before routing to `/messages/[handle]`, so a trade-invite signup migrates, stitches, and fires `signup_completed` (source `"confirm"`).
...
- **Trade-request modal — full post-signup trade flow SHIPPED + prod-verified** (commit stack
  ...
  **UPDATED (16 Jun 2026):** the `onSend` + CTA described above are SUPERSEDED by Message Requests —
  `onSend` now routes through `create_message_request` (no send-time friendship insert; the friendship
  is born at ACCEPT), and the CTA reads "Send message request" / "Send message". The mount, intent
  carrier, and rich-card-tile pieces are unchanged. See "MESSAGE REQUESTS — SHIPPED" below.
...
- **Cross-device onboarding — anonymous collection now survives a sign-up-on-desktop / confirm-on-phone flow** (commits `c9a6f30`, `2dbdbcb`, `c2e029d`). Was device-local: migration read `ms_anon_entries` from the *confirming* device's localStorage, so cross-device signup silently lost the collection AND the analytics identity stitch linked the wrong anon_id. Now:
    - New `pending_anon_stash` table (migration `20260613101130`): email PK (normalised lowercase+trim), entries/set_modes jsonb, anon_id (original signup-device id), expires_at (now()+24h), RLS on / no client policies / service-role only.
    - Write at signup-start: `POST /api/anon-stash` (PUBLIC, unauthenticated), try-caught so it never blocks signup. Merge-on-write: dedup on `${setId}|${cardNumber}|${printingId}`, higher quantity wins, refreshes TTL each write. 500-entry cap.
    - Consume at confirm: `POST /api/anon-stash/consume` (AUTHENTICATED, not public). Two-client split — session/cookie client for auth + user.email lookup + collection_entries/user_sets upserts (same RLS path as /api/anonymous-migration); service client only for stash SELECT + DELETE. Consume-on-migrate deletes the row after success.
    - Identity stitch fix: `identifyOnSignup(userId, explicitAnonId)` now optional-arg; confirm page passes the *stashed* anon_id so the ORIGINAL signup-device session links to user_id. Login signin caller unchanged.
    - Precedence: server stash is source of truth; localStorage is a same-device fallback (runs only when stash migrated 0).
    - Cleanup: pg_cron job `cleanup_pending_anon_stash` (migration `20260613103339`, hourly DELETE of expired rows) + lazy fire-and-forget backstop in the write route.
    - 24h hold warning on the "check your email" screen, gated on whether the user had anonymous entries.
    - Verified in prod (test14, 13 Jun 2026): desktop add 5 → confirm on phone → cards migrated, stash deleted, user_sets upserted, desktop anon_id linked, full anon→signup funnel under one anon_id.

- **Founder analytics instrumentation — set-page + auth-path hooks shipped** (commits from analytics PART 1/2a/2b this session). See the new first-party-analytics note in §1 for the standing picture. Set-page hooks (set_opened, card_added authed+anon, threshold_shown, auth_required_shown, nav_away_warned) and auth-path hooks (signup_started, signup_completed confirm-only, identity stitch) live and prod-verified. EVENTS constant + identifyOnSignup/hasFired/markFired in `lib/track.js`; `/api/analytics/identify` + analytics_identity table (migration `20260613080637`).

- **Email OTP expiry raised 3600 → 86400 (1h → 24h)** via Supabase Dashboard → Auth → Email. The prior 1h confirmation window was too short and likely costing signups across the board. Stash TTL + warning copy are both keyed to this — change one, change all three.
...
## MESSAGE REQUESTS — SHIPPED (Option 1 hold-the-body, 16 Jun 2026)

**Status:** Built, deployed, device-tested. Supersedes the 16-Jun "NEXT BUILD" spec and the
13-Jun parked spec. Replaces the trade-modal's old send-time friend-request model (§18 trade-modal
entry): a non-friend's first contact is now HELD as a request; the recipient ACCEPTS (where the
friendship is born) or DECLINES. ANY non-friend first contact flows through this — the trade-invite
signup is just the first caller.

**Commit stack (oldest→newest, all on `origin/main`):**
- `2c4bd74` — PART 1: `message_requests` table + `create_message_request` / `accept_message_request`
  / `decline_message_request` RPCs + the `notify_new_message` `_suppress_notify` guard. Migration
  applied via MCP + committed together.
- `5d13c5a` — PART 2a: `create_message_request` fast-path tightened to `status = 'accepted'` only
  (a PENDING friendship is NOT a connection → routes through the request gate like a stranger).
- `f888063` — PART 2c/2d: composer `send()` + `/sets` modal `onSend` routed through
  `create_message_request`; **send-time friendship insert REMOVED** (friendship is now born at
  ACCEPT time in RPC 2); CTA copy "Send message request" / "Send message"; toast "Message request
  sent to @handle".
- `06666d7` — PART 3: inbox "Message requests" section (above conversations, only when ≥1 pending)
  + inline accept/decline (reusing the notifications lime-Accept / outline-Decline pattern) +
  `message_request` notification type (MailPlus icon, tappable → `/messages?tab=requests`).
- `57f15e4` — sender-SELECT RLS so a sender can read their OWN outgoing requests (deferred from
  PART 1).
- `04d3321` — sender's thread shows their pending request read-only (green outgoing bubble + card
  tile via the extracted shared `CardStrip` + "Pending — they'll see this if they accept"), gated
  on `messages.length === 0` so it flips to the real delivered message on accept (no double-render).

**LOCKED BEHAVIOUR (so a fresh session doesn't re-derive it):**
- **Option 1 (hold-the-body):** held content lives in `message_requests` (`first_message` +
  `payload` jsonb), NEVER in `messages` until accept. The existing `messages` table, every
  `messages` reader, `get_inbox_threads`, `notify_new_message`, the MSShell badge, both realtime
  subs, and `messages` RLS are ALL UNTOUCHED — the gate is structural, not a filter.
- **Gate:** any pair that is NOT accepted-friends routes through a request; accepted-friends send
  normally. Two send sites gated: composer `send()` (`app/messages/[handle]/page.js`) + `/sets`
  modal `onSend` (`app/sets/page.js`). `/api/trade/propose` is already friends-gated (403 "Not
  friends") — UNTOUCHED.
- **`create_message_request` returns** `blocked` | `friends` | `created` | `updated`. Callers
  branch: `friends` → normal `messages` insert; `created`/`updated` → held (no messages row);
  `blocked` → silent generic failure (never reveal the block).
- **Re-send = REPLACE** (`ON CONFLICT (sender_id, recipient_id) DO UPDATE`, D1), no
  re-notification. Plain `UNIQUE(sender_id, recipient_id)` + delete-on-resolve.
- **Decline (RPC 3):** delete the request row; `friendships` UNTOUCHED; no sender notification; no
  block.
- **Accept (RPC 2, atomic, one transaction):** friendship → accepted (`user_a` = request sender =
  initiator, both directions guarded against the directional UNIQUE) + held content replayed into
  `messages` as UNREAD from the new friend (carries `metadata._suppress_notify` so
  `notify_new_message` skips the redundant ping — the `message_request` notification already fired
  at create time) + request row deleted.
- **Sender pending view:** the sender sees their own held request read-only in the thread (gated on
  an empty thread). On accept, RPC 2 delivers the real message via realtime → `messages.length`
  0→1 → block flips to the real bubble (identical content) and the request row is deleted;
  `pendingRequest` is also cleared on the me→them realtime insert. A re-send updates the block
  optimistically (D1 replace).
- **RLS:** `message_requests_recipient_select` (`auth.uid() = recipient_id`) +
  `message_requests_sender_select` (`auth.uid() = sender_id`); NO client write policies — all
  writes via the three RPCs. All three RPCs SECURITY DEFINER, actor = `auth.uid()` derived
  INTERNALLY (never a param — house security rule), `SET search_path = public`.
- **Notification link** `/messages?tab=requests` lands on `/messages` where the requests section
  sits at the top (no separate tab/route; the param is honoured by position).
...
`20260616140000_message_requests_sender_select.sql` (each applied via MCP + committed together).

---

## TRADE BINDER SHARE — SHIPPED (16 Jun 2026)

Was parked (three share fixes); now done. Share/copy the BARE URL only — the "Check out…" message
rides in the trade-binder page's Open Graph preview, not glued into the copied string (that
concatenation broke paste-into-URL-bar on the iOS sheet).

**Commit stack:**
- `2f45376` — share/copy BARE URL only (dropped `text`/`title` from `navigator.share`; the desktop
  clipboard fallback was already bare); OG description → "Check out {name}'s Pokémon TCG Trade
  Binder on Master Setter." (`app/trade-binder/[handle]/layout.js`); "Link copied" toast on the
  clipboard path (the `tradeToastBar` / `restoreToast` lime bottom-centre pattern).
- `188459f` — extracted `components/ShareTradeBinder.jsx` (SSR-safe: `origin` resolved in a mount
  `useEffect`, null during SSR); moved the share control ABOVE the Wants List on `/you`; added the
  lighter single-button entry point in the owner's trade-binder header (`isOwnPage`, below "Add
  Cards"). The `showQr` prop gates the QR bottom-sheet (folded into the component; only `/you`
  passes it).
- `0b8c740` — toast on successful `navigator.share` resolve ("Shared") so desktop (Windows
  Edge/Chrome now support `navigator.share`, whose sheet gives little feedback) isn't silent;
  AbortError/cancel stays silent. Clipboard path keeps "Link copied".

**OG verified live** (`curl` on `mastersettertcg.com/trade-binder/alex`): `og:description` AND
`twitter:description` both carry the new "Check out…" copy, plus `og:image` (1200×630). A title-only
unfurl on device is platform unfurl-caching or that app rendering large-image cards without a
description line — NOT a bug; OG left untouched. (`proxy.js` already whitelists `/trade-binder/` +
`/brand/`, so unfurl bots fetch both unauthenticated.)

---

## PARKED / OPTIONAL NEXT (small)

- **"Cancel request" affordance** — senders can now SEE their pending request (read-only, in the
  thread) but can't retract it before accept. Natural next addition if wanted; explicitly out of
  scope so far. Would need a `cancel_message_request` RPC (actor = sender, derived from
  `auth.uid()`) + a cancel button on the sender's pending block.
- **Mobile toast double-confirm** — the share toast now fires on BOTH desktop and mobile
  (`navigator.share` resolve). If the mobile OS-sheet + in-app toast feels redundant, gate the
  toast desktop-only (one line in `ShareTradeBinder.handleShare`). Pending Alex's device judgement.

**Conventions still in force (unchanged):** migrations committed WITH their MCP apply; body-only
`CREATE OR REPLACE` needs no DROP; per-part commits with a build between; device-test before "done";
diagnose-first with hard stops.
