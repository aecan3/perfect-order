...
- **Marketplace PART 3 (Discover feed integration)** — surface live listings in the Discover feed tiles. Fetch cached `marketplace_listings` for visible printings, overlay price + "Buy" button on card tiles, trigger background refresh for stale printings via `refreshStaleForUser`. EPN enrolment is Alex's parallel admin task (not blocking — bare `itemWebUrl` is the fallback until `EBAY_EPN_CAMPAIGN_ID` is set).
