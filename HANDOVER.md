...
- **Trade-invite signup gap (analytics 2b-followup):** the propose_trade sub-branch in `/auth/confirm` does no migration and fires no signup_completed — a trade-invite signup confirming via that branch gets neither cards nor the event. Trade-matching is the core differentiator, so highest-value analytics gap to close. NOT done.
