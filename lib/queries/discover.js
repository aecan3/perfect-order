...
export async function getDiscoverMatches({ supabase, viewerUserId, friendIds }) {
  const [{ data: friendEntries }, { data: myHave }] = await Promise.all([
    ...
    // Printings the viewer already owns (checked=true rows only — "no row" means wants)
    supabase
      .from("collection_entries")
      .select("printing_id")
  ...
  const myHavePrintingIds = new Set(
    (myHave || []).map((e) => e.printing_id).filter(Boolean)
  );
