...
async function fetchInFlightKeys(supabase, friendIds) {
  ...
  for (const item of activeItems || []) {
    ...
    // side="offer" → proposer is sending; side="request" → recipient is sending
    const senderId = item.side === "offer" ? trade.proposer_id : trade.recipient_id;
    keys.add(`${senderId}:${item.printing_id}`);
  }
  return keys;
}
