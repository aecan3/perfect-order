...
export function buildEbayUrl({ cardName, cardNumber, setTotal, rarity, userCountry }) {
  ...
  const searchParts = [cardName, numberFormatted, rarityTerm, "Pokemon TCG"].filter(Boolean).join(" ");
  const searchTerm = encodeURIComponent(searchParts);
  return `https://www.${market.domain}/sch/i.html?_nkw=${searchTerm}&mkcid=1&mkrid=705-53470-19255-0&siteid=${market.siteId}&campid=${market.campaignId}&customid=&toolid=10001&mkevt=1`;
}
