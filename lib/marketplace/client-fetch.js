...
/**
 * Invalidate the cached result for a marketplace. Call after a pull-to-refresh
 * so a subsequent GET (e.g. on re-navigation to Discover) fetches fresh data.
 *
 * @param {string} marketplaceId — e.g. "EBAY_AU"
 */
export function clearMarketplaceCache(marketplaceId) {
  recentResults.delete(marketplaceId);
}
