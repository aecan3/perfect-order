...
export async function identifyOnSignup(userId, explicitAnonId = null) {
  if (typeof window === "undefined") return;
  if (!userId || typeof userId !== "string") return;
  // explicitAnonId lets the confirm page link the ORIGINAL signup-device anon_id
  // (carried through the cross-device stash) instead of this device's local one.
  // Existing callers pass nothing and keep using the local anon_id.
  const anon_id = explicitAnonId || getAnonId();
  if (!anon_id) return;
